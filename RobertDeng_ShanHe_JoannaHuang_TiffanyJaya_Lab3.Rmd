---
title: "W271 Lab 3"
author: "Tiffany Jaya, Robert Deng, Joanna Huang, Shan He"
output: 
  pdf_document:
  toc: true
...
Large p-value in the Augmented Dickey-Fuller (ADF) test and small p-value in the Kwiatkowski-Phillips-Schmidt-Shin (KPSS) test confirm our intuition to difference the time series. As suggested by the non-seasonal (ndiffs) and seasonal (nsdiff) unit test, we will perform a non-seasonal difference to the data once and a seasonal difference to the data once in order to make the series stationary in mean. The ADF and KPSS tests we run afterwards validate our differencing decision. We apply log and power transformation to the difference using the Box-Cox transformation by first finding the best lambda value that will give the optimal uniformity in the seasonal variation before administering the said transformation.
...
Looking at the autocorrelation function (ACF) and partial autocorrelation function (PACF) of the second order differenced, we estimate the best-fitting model to be $\text{ARIMA}(1,1,1)(1,1,1)_4$. 

Seasonal D if you see regular oscillations in the series. Regular d if you see and upward or downward trend over time. 
...
By iterating through multiple parameters, we can find the second best-fitting model under the AICc criterion: $\text{ARIMA}(0,1,0)(0,1,2)_4$.
...
We compare our generated best-fitting model to the one generated by the auto.arima function and found it to be the same.

```{r}
(best.auto.m <- auto.arima(BoxCox(sales.train, lambda), ic='aicc', stepwise=FALSE, approximation=FALSE))
```

Although we know that first-order differencing does not pass the unit root test KPSS, we will use the model derived from the auto.arima function as a comparison model. The three models that we will take a look at moving forward are the one we derived manually ($\text{ARIMA}(1,2,1)(1,2,1)_4$), the one we automated manually ($\text{ARIMA}(0,2,1)(0,2,2)_4$) and the one we generated from the auto.arima function ($\text{ARIMA}(0,1,0)(0,1,2)_4$).

**Validating the models**

Before we can forecast what percentage of total retail sales e-commerce sales will be in the future, we first need to validate that the residuals from the three models result in the following properties:
...
```{r fig.show='hold', fig.align='center', out.width='49%'}
checkresiduals(base.m$residuals)
h <- min(2 * 4, nrow(sales.train)/5) # min(2m, T/5) 
Box.test(base.m$residuals, type='Ljung-Box', lag=h) 
shapiro.test(base.m$residuals)
```

```{r fig.show='hold', fig.align='center', out.width='49%'}
checkresiduals(best.manual.m$residuals)
h <- min(2 * 4, nrow(sales.train)/5) # min(2m, T/5) 
...
```{r fig.show='hold', fig.align='center', out.width='49%'}
checkresiduals(best.auto.m$residuals)
h <- min(2 * 4, nrow(sales.train)/5) # min(2m, T/5) 
Box.test(best.auto.m$residuals, type='Ljung-Box', lag=h) 
shapiro.test(best.auto.m$residuals)
```

Looking at the ACF plots, all spikes of the automated models (best.manual.m, best.auto.m) are within the significant limits, meaning that the residuals are uncorrelated to one another. The same cannot be said about our manually generated model (base.m) with one significant spike at lag 8. We then perform a test on a group of autocorrelations with the Box-Ljung test. The Box-Ljung test validates our assumption that the residuals are uncorrelated. With large p-values, the residuals appear to be uncorrelated for all models. The time plot of the residuals shows that the variation of the residuals stays approximately the same for the three models, so we can treat the residual variance as constant. However, even with a mean close to zero, the histogram of all models suggests that it follows more of a negative skewed distribution than normal. The Shapiro-Wilk test confirms the non-normality of the distribution for the automated models (best.manual.m, best.auto.m) and barely do so for the manually generated one (base.m). What this signifies is that when we perform a prediction in the following section, its forecast will generally be quite good but prediction intervals computed assuming a normal distribution may be inaccurate.
...
```{r}
sales.test
InvBoxCox(predict(base.m, 3*4)$pred, lambda)
InvBoxCox(predict(best.manual.m, 3*4)$pred, lambda)
InvBoxCox(predict(best.auto.m, 3*4)$pred, lambda)
```
```{r}
accuracy(forecast(base.m), sales.test)
accuracy(forecast(best.manual.m), sales.test)
accuracy(forecast(best.auto.m), sales.test)
```
```{r fig.show='hold', fig.align='center', out.width='32%'}
ts.plot(cbind(sales.train, sales.test, InvBoxCox(predict(base.m, 4*3)$pred, lambda)), 
...
ts.plot(cbind(sales.train, sales.test, InvBoxCox(predict(best.manual.m, 4*3)$pred, lambda)), 
        col=c('black', 'orange', 'blue'), lty=c(1,1,2), 
        main='Forecasts to 2017 with manual ARIMA(0,2,1)(0,2,2)[4]')
...
plot(forecast(base.m))
plot(forecast(best.manual.m))
plot(forecast(best.auto.m))
```

The forecasts, the RMSE, and the graphs tell us that our generated model $\text{ARIMA}(0,2,1)(0,2,2)_4$ predict much more closely to the test data than the other two. We use our generated model to determine that e-commerce makes up approximately 11.80% of all retail sales by the fourth quarter of 2017.
