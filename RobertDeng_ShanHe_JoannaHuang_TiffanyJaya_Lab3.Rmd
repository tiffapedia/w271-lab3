...
#Read in data
e <- read.csv("./data/ECOMPCTNSA.csv")
...
#train/test
e.train <- ts(ts.e[time(ts.e) < 2015], frequency = 4)
e.test <- ts(ts.e[time(ts.e) > 2014], frequency = 4)
str(e.train)
str(e.test)
...
sum(is.na(e))
```
Ecommerce Sales %'s range from 0.7% to 9.5%, skewing right and mostly aggregated in the lower ranges with 0 NAs. Mean and variance appear to be stationary from the boxplot, but ADF tests can provide more confidence.

```{r fig.show='hold', fig.align='center', out.width='49%'}
plot(e.train, xlab='Date', ylab='E-Commerce Retail Sales', main='E-Commerce Sales: % of Total From 1996-2014')
lines(ma(e.train, order=4, centre=T), col='blue')
```
...
Looking at the time series plot, the data shows strong seasonalality every 4th quarter, and non-stationarity given the upward trend, with increasing variance. Both a seasonal differencing and first-order difference may be needed. The ACF shows a slowly decaying significant positive and negative values that signal a need for higher order differencing. PACF only has 1 significant spike at lag 4 which may suggest an AR order.

The monthplot shows no seasonality, but it comes out in the difference monthplots. Given there is a small increase in the variance with each level from the tsplot, we will also try taking the logarithms to stabilize the variance. To better determine whether differencing is necessary, we will use the ADF test instead of the PP test due to the small sample size of 61.

```{r}
#Not Stationarity, difference between ADF and PP test:
#https://stats.stackexchange.com/questions/14076/phillips-perron-unit-root-test-instead-of-adf-test
adf.test(e.train)
```
ADF test confirms non-stationarity. Now we test out a few transformations (log and difference) to identify the best one to stabilize the stationarity.
...
# Modeling
Given our initial analysis, we look at the model behavior of ARIMA models with these specifications and assumptions as a starting point to pivot from:
  -Varying AR, MA parameters p and q's ranging from 0-3 
  -Log of training data
  -1st seasonal order differencing
  -Seasonal lags 1 for AR and MA for the spikes in the ACF and PACF above (lag 4 reduced by requency 4 = 1)
```{r}
for (q in 0:3){
  for(p in 0:3){
...
```
The model that returned with the smallest AIC value of-205.5 and a BIC of -199.42 was ARIMA(0,1,0)(1,1,1), written as:
(https://www.otexts.org/fpp/8/9)
$(1+0.548\Phi_1B^4)(1-B)(1-B^4)y_t=(1+0.2046\Theta_1B^4)e_t$

```{r}
tm1 <- Arima(log(e.train), order = c(0,1,0),
                         seasonal = list(order = c(1,1,1),4),
                         method = "ML")
summary(tm1)
hist(tm1$residuals)
...
```{r fig.show='hold', fig.align='center', out.width='49%'}
preds = predict(tm1, n.ahead = 8)$pred
ts.plot(e.train,exp(preds), lty = c(1, 3), col = (2))
par(new=TRUE)
ts.plot(ts.e, col="blue", xy.lines = NA)
```
...
```{r}
a <- read.csv("./data/AMAZ.csv")
u <- read.csv("./data/UMCSENT.csv")
#describe(a)
#describe(u)
```
