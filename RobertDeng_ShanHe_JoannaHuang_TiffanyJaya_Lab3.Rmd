...
cbind(adf.test(diff(BoxCox(sales.train, lambda), lag=4, difference=1), alternative='stationary')$p.value, 
      kpss.test(diff(BoxCox(sales.train, lambda), lag=4, difference=1))$p.value)
# unit root test on first-order differenced log-transformed series
cbind(adf.test(diff(BoxCox(sales.train, lambda), lag=4, difference=2), alternative='stationary')$p.value, 
      kpss.test(diff(BoxCox(sales.train, lambda), lag=4, difference=2))$p.value)
```
...
By iterating through multiple parameters, we can find the second best-fitting model under the AICc criterion: $\text{ARIMA}(0,2,1)(0,2,2)_4$.
...
```{r fig.show='hold', fig.align='center', out.width='49%'}
tsdisplay(base.m$residuals)
h <- min(2 * 4, nrow(sales.train)/5) # min(2m, T/5) 
Box.test(base.m$residuals, type='Ljung-Box', lag=h) 
hist(base.m$residuals, main='Histogram of residuals (base.m)')
abline(v=mean(base.m$residuals), col='blue')
...
hist(best.auto.m$residuals, main='Histogram of residuals (best.auto.m)')
