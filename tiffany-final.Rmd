...
The first step once we obtained the dataset was to see if there was any missing value.

```{r}
kable(summary(raw.sales)) # output: no missing value
sales <- ts(raw.sales$ECOMPCTNSA, start=c(1999,4), frequency=4)
head(sales); tail(sales)
```

From the summary table, we were able to determine that there was no missing value with sales appearing to increase  overtime from 0.7% in the 4th quarter of 1999 to 9.5% in the 4th quarter of 2016. To confirm, we plot the time series as well as its associating MA(4) model. Since the series is quarterly, the MA(4) model acts as an annual trend with the seasonal effects within each quarter removed.
...
Clearly, the series is non-stationarity with a strong seasonality. 


The series is a quarterly series. 
